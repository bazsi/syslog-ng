...
_map_http_status_to_worker_status(HTTPDestinationWorker *self, const gchar *url, glong http_code)
{
  HTTPDestinationDriver *owner = (HTTPDestinationDriver *) self->super.owner;
...
_flush_on_target(HTTPDestinationWorker *self, HTTPLoadBalancerTarget *target)
{
  ...
  return _map_http_status_to_worker_status(self, target->url, http_code);
}
